...
def plot_results(results, goals, it, rmin, rmax, marker, logdir):
    ...
    for k in range(marker):
        ...
        if score < rmin:
            ...
            plt.scatter(gx, gy, c='r')
    plt.savefig(logdir+'/gan_goals_'+str(it)+'.png')
    plt.close()

def plot_goals(goals, marker, it, logdir):
    for k in range(len(goals)):
        gx = goals[k][0]
        gy = goals[k][1]
        plt.scatter(gx, gy, c='b')
    plt.savefig(logdir+'/unfiltered_goals_'+str(it)+'.png')
    plt.close()
    for k in range(marker, len(goals)):
        gx = goals[k][0]
        gy = goals[k][1]
        plt.scatter(gx, gy, c='b')
    plt.savefig(logdir+'/unfiltered_gan_goals_'+str(it)+'.png')
    plt.close()
...
def train(env_name,
    ...
    for it in range(max_iters):
        ...
        #index: from here in cand_goals are all generated by GAN
        # if reverse, index from 0 to num_goals-marker are generated by GAN
        marker = len(cand_goals)
        cand_goals.extend(lsgan.generate_goals(num_goals-len(cand_goals)))
        plot_goals(np.array(cand_goals), marker, it, logdir) 
        cand_goals = list(reversed(cand_goals))
        cand_goals = np.array(cand_goals)

        filtered_goals = []
        for gyj in range(len(cand_goals)):
            goal = cand_goals[gyj]
            if gyj==(num_goals-1) - marker:
                #index: from here in filtered goals are all generated by GAN
                new_marker = len(filtered_goals)
            novel = True
        ...
        plot_results(results, goals, it, r_min, r_max, new_marker,logdir)
